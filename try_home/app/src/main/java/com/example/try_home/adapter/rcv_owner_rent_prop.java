...
public class rcv_owner_rent_prop extends RecyclerView.Adapter<rcv_owner_rent_prop.ViewHolder> {
    private Context context;
    private int O_ID;

    // private int oid=sp.getString("own_login_oid",null);

    public rcv_owner_rent_prop(Context context) {
    ...
    @NonNull
    @Override
    public rcv_owner_rent_prop.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.owner_up_card_view,parent,false);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull rcv_owner_rent_prop.ViewHolder holder, int position) {

        /*SqlOp x1=new SqlOp(context.getApplicationContext());
        Cursor c=x1.rcv_O_rent_personal_count(O_ID);
        if(position<c.getCount());
        {
            holder.tv_Prod_ID.setVisibility(View.INVISIBLE);
            holder.tv_bhk.setVisibility(View.INVISIBLE);
            holder.tv_city.setVisibility(View.INVISIBLE);
            holder.tv_price.setVisibility(View.INVISIBLE);
            holder.tv_area.setVisibility(View.INVISIBLE);
        ...
        ArrayList al=x.rcv_O_rent_personal(O_ID,position);
        ...
        holder.ib.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                boolean r=obj.rcv_O_rent_personal_delete(String.valueOf(al.get(0)));
    ...
    public int getItemCount() {
        SqlOp x=new SqlOp(context.getApplicationContext());
        Cursor c=x.rcv_O_rent_personal_count(O_ID);
        return c.getCount();
    }
